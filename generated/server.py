...
async def lidarr_grab_album(
    ...
    # Auto-detect defaults from API if still unset
    if qualityProfileId is None or rootFolderPath is None:
        try:
            _root_folders = await _client.request("GET", "/api/v1/rootfolder")
            if isinstance(_root_folders, list) and len(_root_folders) == 1:
                _rf = _root_folders[0]
                if rootFolderPath is None:
                    rootFolderPath = _rf.get("path")
                if qualityProfileId is None:
                    qualityProfileId = _rf.get("defaultQualityProfileId")
            elif isinstance(_root_folders, list) and len(_root_folders) > 1:
                _options = [{"path": rf.get("path"), "id": rf.get("id")} for rf in _root_folders if isinstance(rf, dict)]
                if rootFolderPath is None:
...
@mcp.tool()
async def lidarr_search_album(
    artist: str,
    album: str = "",
) -> dict[str, Any]:
    """Search for albums by artist name, optionally filtering by album title.

    Looks up the artist via Lidarr's search API and returns their album list from MusicBrainz metadata.
    artist: Artist name to search for (e.g. "Radiohead").
    album: Optional album title substring filter (case-insensitive, e.g. "Kid A").
    Returns matching albums with title, releaseDate, foreignAlbumId, trackCount.
    Use this to browse albums before calling lidarr_grab_album. For MusicBrainz ID lookups, use lidarr_lookup_album with term=lidarr:MBID.
    If unexpected errors occur, call lidarr_report_issue.
    """
    try:
        lookup = await _client.request("GET", "/api/v1/artist/lookup", params={"term": _normalize_unicode(artist)})
    ...
    if not isinstance(lookup, list) or len(lookup) == 0:
        return {"error": True, "message": f"No results for artist '{artist}'."}

    best = lookup[0]
    artist_name = best.get("artistName", artist)
    foreign_artist_id = best.get("foreignArtistId", "")
    raw_albums = best.get("albums", [])
    ...
    for alb in raw_albums:
        if not isinstance(alb, dict):
            continue
        title = alb.get("title", "")
        if album_lower and album_lower not in title.lower():
            continue
        results.append({
            "title": title,
            "foreignAlbumId": alb.get("foreignAlbumId", ""),
            "releaseDate": alb.get("releaseDate", ""),
...
@mcp.tool()
async def lidarr_set_album_release(
    albumId: int,
    releaseIndex: int | None = None,
    confirm: bool = False,
) -> dict[str, Any]:
    """List or select a specific release for an album (e.g. CD vs vinyl, different countries).

    Requires confirm=True to execute a release selection. Set confirm=False to list available releases.
    albumId: The album ID (from Lidarr, not MusicBrainz).
    releaseIndex: Index of the release to select (from the list). Omit to list available releases.
    confirm: Must be True to change the selected release.
    Returns available releases when releaseIndex is omitted. Selects the release when releaseIndex is provided.
    If unexpected errors occur, call lidarr_report_issue.
    """
    try:
        album_data = await _client.request("GET", f"/api/v1/album/{albumId}")
    except httpx.HTTPStatusError as exc:
        return {"error": True, "source": "lidarr_api", "status": exc.response.status_code, "message": exc.response.text[:500], "tool": "lidarr_set_album_release"}
    except Exception as exc:
        return {"error": True, "message": str(exc)[:500]}
    ...
    if not releases:
        return {"error": True, "message": "No releases found for this album."}
    ...
    for i, rel in enumerate(releases):
        ...
        release_list.append({
            "index": i,
            "title": rel.get("title", ""),
            "trackCount": rel.get("trackCount", 0),
            "format": " + ".join(
                f"{m.get('mediumNumber', '?')}: {m.get('mediumFormat', '?')} ({m.get('mediumName', '')})"
                for m in rel.get("media", []) if isinstance(m, dict)
            ) if rel.get("media") else "",
            "country": " / ".join(rel.get("country", [])) if isinstance(rel.get("country"), list) else rel.get("country", ""),
            "monitored": rel.get("monitored", False),
        })

    if releaseIndex is None:
        return {
            "albumId": albumId,
            "albumTitle": album_data.get("title", ""),
            "releaseCount": len(release_list),
            "releases": release_list,
        }

    if not confirm:
        return {"preview": f"Select release {releaseIndex} for album {albumId}", "confirm": "Set confirm=True to execute."}

    if releaseIndex < 0 or releaseIndex >= len(releases):
        return {"error": True, "message": f"releaseIndex {releaseIndex} out of range (0-{len(releases)-1})."}

    # Set monitored=True on selected release, False on all others
    for i, rel in enumerate(releases):
        if isinstance(rel, dict):
            rel["monitored"] = (i == releaseIndex)

    album_data["releases"] = releases

    try:
        await _client.request("PUT", f"/api/v1/album/{albumId}", json_body=album_data)
    except httpx.HTTPStatusError as exc:
        return {"error": True, "source": "lidarr_api", "status": exc.response.status_code, "message": exc.response.text[:500], "tool": "lidarr_set_album_release"}
    except Exception as exc:
        return {"error": True, "message": str(exc)[:500]}

    selected = release_list[releaseIndex] if releaseIndex < len(release_list) else {}
    return {
        "ok": True,
        "albumId": albumId,
...
if _module_enabled("album"):
    ...
    async def lidarr_lookup_album(
        ...
        """Lookup album. Returns a list. If unexpected errors occur, call lidarr_report_issue. Note: This endpoint only works with MusicBrainz release group IDs (term=lidarr:MBID). For text-based album search, use lidarr_search_album instead.
...
if _module_enabled("album"):
    if not LIDARR_READ_ONLY:
        ...
        async def lidarr_update_album(
            ...
            """Update album by ID If unexpected errors occur, call lidarr_report_issue. Note: Call lidarr_create_command with name='RefreshArtist' after updating. Uses merge=True by default to auto-fetch the current object. For batch album monitoring, use lidarr_update_albums_monitored instead. To select a specific release, use lidarr_set_album_release.
...
if _module_enabled("artist"):
    if not LIDARR_READ_ONLY:
        ...
        async def lidarr_create_artist(
            ...
            """Create artist If unexpected errors occur, call lidarr_report_issue. Note: Use the 'monitor' shorthand param instead of addOptions.monitor. Valid values: all, future, missing, existing, latest, first, none. Default monitors entire discography. Warning: monitor='none' may still mark albums as monitored (Lidarr API bug). Workaround: create artist, then batch-unmonitor albums, then selectively monitor. Or use lidarr_grab_album which handles this automatically. foreignArtistId accepts MusicBrainz artist IDs for direct creation when Lidarr search API is unavailable. Use lidarr_search_album to browse an artist's albums before adding.
            ...
            monitorNewItems: Values: all, none, new
            monitor: Shorthand for addOptions.monitor. Valid values: all, future, missing, existing, first, latest, none. When set, injects into addOptions automatically.
            """
            if not confirm:
            ...
            if rootFolderPath is None and LIDARR_DEFAULT_ROOT_FOLDER:
                rootFolderPath = LIDARR_DEFAULT_ROOT_FOLDER
            # Auto-detect qualityProfileId/rootFolderPath from API if still unset
            if qualityProfileId is None or rootFolderPath is None:
                try:
                    _root_folders = await _client.request("GET", "/api/v1/rootfolder")
                    if isinstance(_root_folders, list) and len(_root_folders) == 1:
...
if _module_enabled("command"):
    if not LIDARR_READ_ONLY:
        ...
        async def lidarr_create_command(
            ...
            wait_timeout: Max seconds to wait when wait=True (default 120).
            Returns compact response: single objects have nested data summarized, batch results return {"ok": true, "count": N, "ids": [...]}.
...
if _module_enabled("command"):
    if not LIDARR_READ_ONLY:
        ...
        @mcp.tool()
        async def lidarr_command_rename_files(
            artistId: int,
            files: list[int],
            confirm: bool = False,
            wait: bool = False,
            ...
            """Rename track files for an artist according to naming settings. Set wait=True to poll until completion (default timeout 120s, may be slow on NFS). If unexpected errors occur, call lidarr_report_issue.

            Requires confirm=True to execute. Set confirm=False to preview.
            wait: If True, poll until the command completes, fails, or times out (default False).
            wait_timeout: Max seconds to wait when wait=True (default 30).
            Returns compact response: single objects have nested data summarized, batch results return {"ok": true, "count": N, "ids": [...]}.
            artistId: The artist ID whose files to rename.
            files: List of track file IDs to rename.
            """
            if not confirm:
                return {"preview": "POST /api/v1/command", "confirm": "Set confirm=True to execute this POST request."}
            _path = "/api/v1/command"
            _body: dict[str, Any] = {"name": "RenameFiles"}
            _body["artistId"] = artistId
            _body["files"] = files
            try:
                _resp = await _client.request(
                ...
                return {
                    ...
                    "status": exc.response.status_code,
                    "message": exc.response.text[:500],
                    "tool": "lidarr_command_rename_files",
                }
            except httpx.RequestError as exc:
                return {
                    "error": True,
...
if _module_enabled("command"):
    if not LIDARR_READ_ONLY:
        ...
        @mcp.tool()
        async def lidarr_command_rename_artist(
            artistIds: list[int],
            confirm: bool = False,
            wait: bool = False,
            wait_timeout: int = 30,
        ) -> dict[str, Any] | list[Any] | str:
            """Rename artist folders according to naming settings. Set wait=True to poll until completion (default timeout 120s, may be slow on NFS). If unexpected errors occur, call lidarr_report_issue.

            Requires confirm=True to execute. Set confirm=False to preview.
            wait: If True, poll until the command completes, fails, or times out (default False).
            wait_timeout: Max seconds to wait when wait=True (default 30).
            Returns compact response: single objects have nested data summarized, batch results return {"ok": true, "count": N, "ids": [...]}.
            artistIds: List of artist IDs whose folders to rename.
            """
            if not confirm:
                return {"preview": "POST /api/v1/command", "confirm": "Set confirm=True to execute this POST request."}
            _path = "/api/v1/command"
            _body: dict[str, Any] = {"name": "RenameArtist"}
            _body["artistIds"] = artistIds
            try:
                _resp = await _client.request(
                    "POST",
                ...
                return {
                    ...
                    "status": exc.response.status_code,
                    "message": exc.response.text[:500],
                    "tool": "lidarr_command_rename_artist",
                }
            except httpx.RequestError as exc:
                return {
                    "error": True,
